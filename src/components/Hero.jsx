...
const ROTATIONS = [
  { rotate: "-rotate-6", z: "z-10", offset: "translate-x-0 translate-y-2" },
  { rotate: "rotate-0",  z: "z-20", offset: "translate-x-0 translate-y-0" },
  { rotate: "rotate-6",  z: "z-10", offset: "translate-x-0 translate-y-2" },
];
...
export default function Hero() {
  ...
  const [featuredTitle, setFeaturedTitle] = useState("Dragon's Ascent");
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data?.length) {
          setCovers(
            data.map((m, i) => ({
              src: m.cover_url || `https://picsum.photos/seed/hero${i + 1}/180/260`,
              ...ROTATIONS[i],
            }))
          );
          setFeaturedTitle(data[0]?.title ?? featuredTitle);
        }
      });
  }, []);
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden pt-16">
      {/* Background glows */}
      <div className="hero-glow w-96 h-96 bg-manga-red top-20 -left-20" style={{ position: 'absolute' }} />
      <div className="hero-glow w-80 h-80 bg-manga-purple bottom-20 right-10" style={{ position: 'absolute' }} />
      <div className="hero-glow w-64 h-64 bg-manga-orange top-1/2 left-1/2" style={{ position: 'absolute' }} />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24 w-full">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          ...
          {/* Visual: manga stack from Supabase */}
          <div className="flex-shrink-0 relative flex items-center justify-center w-full lg:w-auto">
            <div className="relative flex items-end justify-center gap-2 h-72 sm:h-80 lg:h-96">
              {covers.map((m, i) => (
                <div key={i} className={`relative ${m.z} transform ${m.rotate} ${m.offset} card-hover`}>
                  <img
                    src={m.src}
                    alt="Manga cover"
              ...
              <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 bg-manga-card border border-manga-border rounded-xl px-4 py-2.5 shadow-xl whitespace-nowrap">
                <p className="text-xs text-manga-muted">Más popular ahora</p>
                <p className="text-sm font-bold text-white">{featuredTitle}</p>
              </div>
            </div>
          </div>
        </div>
